...
async function loadSettings(){const s=await get("settings")||{},asset=await get("characterSourceAsset");$("#autoRead").checked=!!s.autoRead;$("#reduceMotion").checked=!!s.reduceMotion;document.documentElement.classList.toggle("reduceMotion",!!s.reduceMotion);$("#characterSummary").textContent=s.characterName?`탐험가 · ${s.characterName}`:"Character Master 기록 보존";$("#guideSummary").textContent=`말티푸 · ${s.guideName||"모카"}`;$("#characterName").value=s.characterName||"";$("#guideName").value=s.guideName||"모카";if(asset?.originalProfilePhoto){$("#profilePhotoPreview").hidden=false;$("#profilePhotoImage").src=asset.originalProfilePhoto;$("#profilePhotoStatus").textContent=asset.characterMasterId?"Character Master 연결됨":"원본 사진 준비됨 · MASTER 스타일 생성 연결 대기"}else{$("#profilePhotoPreview").hidden=true;$("#profilePhotoStatus").textContent="원본 사진 없음"}}
...
$("#profilePhotoInput").onchange=async e=>{const file=e.target.files?.[0];if(!file)return;if(!file.type.startsWith("image/"))return toast("이미지 파일만 사용할 수 있어요.");const reader=new FileReader();reader.onload=async()=>{const data=String(reader.result||""),previous=await get("characterSourceAsset"),history=await get("characterSourceHistory")||[];if(previous?.originalProfilePhoto)history.push({...previous,archivedAt:new Date().toISOString()});const asset={assetId:uid("profile_source"),originalProfilePhoto:data,processedProfilePhoto:null,characterMasterId:previous?.characterMasterId||null,status:"SOURCE_READY",createdAt:new Date().toISOString(),sourceName:file.name||"camera"};await setMany([["characterSourceAsset",asset],["characterSourceHistory",history]]);$("#profilePhotoPreview").hidden=false;$("#profilePhotoImage").src=data;$("#profilePhotoStatus").textContent="원본 사진 준비됨 · MASTER 스타일 생성 연결 대기";toast("원본 사진을 보존했어요. 기존 기록은 유지됩니다.")};reader.onerror=()=>toast("사진을 읽지 못했어요.");reader.readAsDataURL(file)};
$("#characterSave").onclick=async()=>{const name=$("#characterName").value.trim();const s=await get("settings")||{};s.characterName=name;await set("settings",s);$("#characterSummary").textContent=name?`탐험가 · ${name}`:"Character Master 기록 보존";$("#characterPanel").hidden=true;toast("캐릭터 설정을 저장했어요. 원본 사진과 기록은 분리 보존됩니다.")};
