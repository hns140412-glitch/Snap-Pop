...
async function renderIdentityPresence(){if(!db)return;const identity=await resolvedIdentity(),visual=$("#homeCharacterVisual"),rule=crewMemberRule(identity),name=crewMemberName(identity);$("#homeCharacterName").textContent=identity.profile.name||"나의 탐험가";$("#homeCrewMemberName").textContent=name;$("#homeCrewMemberLine").textContent=rule.home||"";const guideVisual=$("#homeCrewMember .crewMemberPlaceholder");if(guideVisual)guideVisual.textContent=rule.label;if(identity.profile.photo){visual.innerHTML=`<img src="${identity.profile.photo}" alt="">`}else visual.textContent=(identity.profile.name||"탐험가").slice(0,2);$("#growthCompanion").textContent=`${identity.profile.name||"탐험가"} · ${name}와 함께 성장 중`;$("#crewMemberPersonalityPreview").textContent=`${rule.label} · ${rule.personality||""}`}
...
$$("#crewMemberTypes [data-crew-member-type]").forEach(b=>b.onclick=async()=>{const identity=await resolvedIdentity();identity.crewMember.type=normalizeCrewType(b.dataset.crewMemberType);const rule=crewMemberRule(identity);identity.crewMember.name=rule.defaultName||identity.crewMember.name||"모카";identity.explorationCrewRulesVersion=SNAP_RULES.version;await set("identityFallback",identity);$$("[data-crew-member-type]").forEach(x=>x.classList.toggle("on",x.dataset.crewMemberType===identity.crewMember.type));$("#crewMemberName").value=identity.crewMember.name;$("#crewMemberPersonalityPreview").textContent=`${rule.label} · ${rule.personality||""}`;await renderIdentityPresence()});
