...
$("#nextBtn").onclick=async()=>{let s=await get("active");if(!s){toast("지도에서 탐험지를 먼저 골라줘.");show("map");return}const i=s.step||0;s.answers[i]=$("#answer").value.trim();if(!s.answers[i])return toast("한 줄이라도 남겨볼까?");if(i<2){s.step=i+1;await set("active",s);renderExplore(s);return}const r=await get("records")||[];r.unshift({id:Date.now(),landmark:s.landmark,answers:s.answers,date:new Date().toISOString()});await set("records",r);const g=await get("gems")||{};g[s.landmark]=(g[s.landmark]||0)+1;await set("gems",g);await set("exp",(await get("exp")||0)+55);await set("active",null);await updateStatus();toast("탐험 완료! 보석 조각과 성장이 기록됐어.");show("growth")}
...
$("#guideBtn").onclick=()=>toast("현재 길잡이 디자인 계보는 유지하고 이름 변경만 연결할 예정이에요.");
