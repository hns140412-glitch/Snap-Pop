...
const IDENTITY_DEFAULT={profile:{name:"",photo:"",style:"editorial",shareAvatar:false},guide:{type:"lumi",name:"루미",voice:"warm"}};
...
async function resolvedIdentity(){return normalizeIdentity(sharedIdentity||await get("identityFallback")||await migrateIdentityFallback())}
async function applySharedIdentity(identity){sharedIdentity=normalizeIdentity(identity);await renderIdentityPresence();return sharedIdentity}
...
async function renderIdentityPresence(){if(!db)return;const identity=await resolvedIdentity(),visual=$("#homeCharacterVisual");$("#homeCharacterName").textContent=identity.profile.name||"나의 탐험가";$("#homeGuideName").textContent=identity.guide.name||"루미";if(identity.profile.photo){visual.innerHTML=`<img src="${identity.profile.photo}" alt="">`}else visual.textContent=(identity.profile.name||"탐험가").slice(0,2);$("#growthCompanion").textContent=`${identity.profile.name||"탐험가"} · ${identity.guide.name||"루미"}와 함께 성장 중`}
