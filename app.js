...
async function renderCrewRosters(){
  if(!SNAP_RULES)return;
  const identity=await resolvedIdentity(),starter=SNAP_RULES.roster?.starterMainCandidates||{},defined=starter.definedMembers||[],max=starter.max||6,min=starter.min||5;
  const starterHost=$("#starterCrewRoster"),worldHost=$("#worldCrewRoster"),specialHost=$("#specialCrewRoster");
  if(starterHost){
    const definedCards=defined.map(id=>{const m=SNAP_RULES.members?.[id];if(!m)return "";const on=identity.crewMember.type===id;return `<button type="button" class="crewRosterCard ${on?"on":""}" data-crew-member-type="${id}"><b>${html(m.label)}</b><span>${html(m.personality||"")}</span></button>`}).join("");
    const openCount=Math.max(0,max-defined.length);
    const openCards=Array.from({length:openCount},(_,i)=>`<button type="button" class="crewRosterCard unassigned" disabled><b>후보 미확정</b><span>시작 ${defined.length+i+1}번 슬롯 · 정체성 OPEN</span></button>`).join("");
    starterHost.innerHTML=definedCards+openCards;
    starterHost.setAttribute("aria-label",`시작 탐험대 후보 ${min}~${max}명`);
  }
  if(worldHost){worldHost.innerHTML=`<div class="crewRosterCard unassigned"><b>거점·지역 탐험대원</b><span>고정 5거점의 역할 필요성으로 인원 산정 · 숫자 OPEN</span></div>`}
  if(specialHost){specialHost.innerHTML=`<div class="crewRosterCard locked"><b>미발견 스페셜</b><span>최대 ${SNAP_RULES.roster?.special?.max||12}명 · 흔적/그림자 → 조우 → 도감 공개</span></div>`}
  $("#crewMemberPersonalityPreview").textContent=`${crewMemberRule(identity).label} · ${crewMemberRule(identity).personality||""}`;
}
async function chooseStarterCrewMember(type){
  if(!(SNAP_RULES.roster?.starterMainCandidates?.definedMembers||[]).includes(type))return toast("아직 확정되지 않은 탐험대원이에요.");
  const identity=await resolvedIdentity();identity.crewMember.type=normalizeCrewType(type);const rule=crewMemberRule(identity);
  identity.crewMember.name=rule.defaultName||identity.crewMember.name||"모카";identity.explorationCrewRulesVersion=SNAP_RULES.version;
  await set("identityFallback",identity);$("#crewMemberName").value=identity.crewMember.name;await renderCrewRosters();await renderIdentityPresence();
}
async function init(){await openDB();await migrateLegacyState();SNAP_RULES=await fetch("data/exploration-crew-rules.json").then(r=>r.json());marks=await fetch("data/landmarks.json").then(r=>r.json());await migrateIdentityFallback();renderLandmarks();await loadSettings();await renderCrewRosters();await renderIdentityPresence();await updateStatus();renderRecords();renderGems();renderGrowth();await renderIncomingHandoff();renderSpecialInvite();const active=await get("active");if(active)renderExplore(active)}
...
async function renderIdentityPresence(){if(!db)return;const identity=await resolvedIdentity(),visual=$("#homeCharacterVisual"),rule=crewMemberRule(identity),name=crewMemberName(identity);$("#homeCharacterName").textContent=identity.profile.name||"나의 탐험가";$("#homeCrewMemberName").textContent=name;$("#homeCrewMemberLine").textContent=rule.home||"";const memberVisual=$("#homeCrewMember .crewMemberPlaceholder");if(memberVisual)memberVisual.textContent=rule.label;if(identity.profile.photo){visual.innerHTML=`<img src="${identity.profile.photo}" alt="">`}else visual.textContent=(identity.profile.name||"탐험가").slice(0,2);$("#growthCompanion").textContent=`${identity.profile.name||"탐험가"} · ${name}와 함께 성장 중`;$("#crewMemberPersonalityPreview").textContent=`${rule.label} · ${rule.personality||""}`}
...
async function renderLastResult(){const r=await get("lastResult");if(!r){$("#resultTitle").textContent="아직 완료한 탐험이 없어요.";$("#resultDraft").textContent="지도에서 탐험을 시작해봐요.";$("#resultExp").textContent="+0 EXP";$("#resultGem").textContent="보석 조각 +0";$("#resultStrengths").innerHTML="";$("#resultCrewMemberLine").textContent="";$("#bonusStart").disabled=true;return}const m=marks.find(x=>x.id===r.landmark),bonusEvents=await get("bonusEvents")||{},identity=await resolvedIdentity();$("#resultTitle").textContent=m?.title||"오늘의 탐험";$("#resultDraft").textContent=r.answers.join(" ");$("#resultExp").textContent=`+${r.expAward||0} EXP`;$("#resultGem").textContent="보석 조각 +1";$("#resultStrengths").innerHTML=(r.strengths||[]).length?(r.strengths||[]).map(s=>`<span>${html(s)}</span>`).join(""):'<span>내 문장으로 끝까지 완성하기</span>';$("#resultCrewMemberLine").textContent=`${crewMemberName(identity)} · 네가 쓴 건 네 글이야. 나는 옆에서 길만 같이 봤어.`;const done=!!bonusEvents[`bonus_${r.completionEventId}`];$("#bonusStart").disabled=done;$("#bonusStart").textContent=done?"추가 연습 완료됨":"추가 연습";$("#bonusPanel").hidden=true}
...
$("#crewMemberBtn").onclick=()=>$("#crewMemberPanel").hidden=!$("#crewMemberPanel").hidden;
...
$("#starterCrewRoster").onclick=e=>{const b=e.target.closest("button[data-crew-member-type]");if(b)chooseStarterCrewMember(b.dataset.crewMemberType)};
