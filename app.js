...
function renderExplore(s){const m=marks.find(x=>x.id===s.landmark)||marks[0],i=Math.min(2,s.step||0),language=s.language||"ko",p=promptFor(s.landmark,i,language);$("#exploreTitle").textContent="탐험 진행 · "+m.title;$("#question").textContent=p[0];$("#hint").textContent=p[1];$("#answer").value=s.answers[i]||"";setModeButtons(language);resolvedIdentity().then(identity=>{const name=crewMemberName(identity),base=crewReaction(identity,s.landmark),stepLine=language==="en"?["Start small. One idea is enough.","Add one more piece.","Finish it in your own words."][i]:["작은 조각 하나부터 잡아보자.","좋아, 하나만 더 붙여보자.","이제 네 말로 마무리해보자."][i];$(".guideLine b").textContent=`탐험대원 ${name}`;$("#guideLine").textContent=`${base} ${stepLine}`});$("#nextBtn").textContent=i===2?(language==="en"?"Finish exploration":"탐험 완료"):(language==="en"?"Next step":"다음 단계");$("#steps").innerHTML=STEPS.map((x,n)=>`<span class="${n===i?"on":n<i?"done":""}">${n+1}. ${x}</span>`).join("")}
...
$("#cloudBtn").onclick=()=>{const panel=$("#cloudPanel"),chips=$("#cloudChips"),frags=cloudFragments($("#answer").value);chips.innerHTML=frags.map(x=>`<button type="button">${html(x)}</button>`).join("");panel.hidden=false;chips.onclick=e=>{const b=e.target.closest("button");if(!b)return;resolvedIdentity().then(identity=>toast(`${crewMemberName(identity)}: 힌트는 여기까지. 이제 네 문장으로 이어가봐.`))}};
...
$$("#guideTypes [data-guide-type]").forEach(b=>b.onclick=async()=>{const identity=await resolvedIdentity();identity.crewMember.type=normalizeCrewType(b.dataset.guideType);const rule=crewMemberRule(identity);identity.crewMember.name=rule.defaultName||identity.crewMember.name||"모카";identity.explorationCrewRulesVersion=SNAP_RULES.version;await set("identityFallback",identity);$$("[data-guide-type]").forEach(x=>x.classList.toggle("on",x.dataset.guideType===identity.crewMember.type));$("#guideName").value=identity.crewMember.name;$("#guidePersonalityPreview").textContent=`${rule.label} · ${rule.personality||""}`;await renderIdentityPresence()});
$("#guideSuggest").onclick=async()=>{const identity=await resolvedIdentity(),suggestions={maltipoo:["모카","토리","콩"],cat:["루루","모노","살짝"],redpanda:["포포","단추","뒤적"],buddy:["하루","담이","솔"]},arr=suggestions[identity.crewMember.type]||[crewMemberRule(identity).defaultName||"모카"];$("#guideName").value=arr[Math.floor(Date.now()/1000)%arr.length]};
$("#guideSave").onclick=async()=>{const identity=await resolvedIdentity(),name=$("#guideName").value.trim()||crewMemberRule(identity).defaultName||"모카";identity.crewMember.name=name;identity.explorationCrewRulesVersion=SNAP_RULES.version;await set("identityFallback",identity);$("#guideSummary").textContent=`${crewMemberRule(identity).label} · ${name}`;$$(".guideLine b").forEach(el=>el.textContent=`탐험대원 ${name}`);$("#guidePanel").hidden=true;await renderIdentityPresence();toast("탐험대원 설정을 저장했어요.")};
