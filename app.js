...
async function renderIdentityPresence(){if(!db)return;const identity=await resolvedIdentity(),visual=$("#homeCharacterVisual");$("#homeCharacterName").textContent=identity.profile.name||"나의 탐험가";const rule=guideRule(identity);$("#homeGuideName").textContent=guideDisplayName(identity);const guideVisual=$("#homeGuide .guidePlaceholder");if(guideVisual)guideVisual.textContent=rule.label;if(identity.profile.photo){visual.innerHTML=`<img src="${identity.profile.photo}" alt="">`}else visual.textContent=(identity.profile.name||"탐험가").slice(0,2);$("#growthCompanion").textContent=`${identity.profile.name||"탐험가"} · ${guideDisplayName(identity)}와 함께 성장 중`}
