...
function normalizeIdentity(x={}){
  ...
  return {profile:{...IDENTITY_DEFAULT.profile,...p},crewMember:{...IDENTITY_DEFAULT.crewMember,...member,type}};
}
...
async function applySharedIdentity(identity){sharedIdentity=normalizeIdentity({profile:identity?.profile||identity,guide:IDENTITY_DEFAULT.crewMember});await renderIdentityPresence();return sharedIdentity}
...
$("#guideSave").onclick=async()=>{const name=$("#guideName").value.trim()||"모카";const s=await get("settings")||{};s.guideName=name;await set("settings",s);$("#guideSummary").textContent=`말티푸 · ${name}`;$(".guideLine b").forEach(el=>el.textContent=`탐험대원 ${name}`);$("#guidePanel").hidden=true;toast("탐험대원 이름을 저장했어요.")};
