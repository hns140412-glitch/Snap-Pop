...
const IDENTITY_DEFAULT={profile:{name:"",photo:"",style:"editorial",shareAvatar:false},guide:{type:"maltipoo",name:"모카",voice:"warm"}};
let sharedIdentity=null;
function normalizeGuideType(type){return ({lumi:"maltipoo",pico:"redpanda",mori:"buddy"}[type]||type||"maltipoo")}
function normalizeIdentity(x={}){
  const p=x.profile||{},g=x.guide||{},type=normalizeGuideType(g.type);
  return {profile:{...IDENTITY_DEFAULT.profile,...p},guide:{...IDENTITY_DEFAULT.guide,...g,type}};
}
function guideRule(identity){const type=normalizeGuideType(identity?.guide?.type);return SNAP_RULES?.guides?.[type]||SNAP_RULES?.guides?.maltipoo||{label:"길잡이",defaultName:"모카",home:"같이 가자.",reactions:{}}}
function guideReaction(identity,landmark){return guideRule(identity).reactions?.[landmark]||"한 조각씩 같이 찾아보자."}
function guideDisplayName(identity){return identity?.guide?.name||guideRule(identity).defaultName||"모카"}
...
async function migrateIdentityFallback(){
  ...
  const migrated=normalizeIdentity({profile:{name:s.characterName||"",photo:asset?.originalProfilePhoto||"",style:"editorial",shareAvatar:false},guide:{type:s.guideType||"lumi",name:s.guideName||"루미",voice:s.guideVoice||"warm"}});
  await set("identityFallback",migrated);
  return migrated;
}
async function resolvedIdentity(){const local=normalizeIdentity(await get("identityFallback")||await migrateIdentityFallback());if(!sharedIdentity)return local;const shared=normalizeIdentity(sharedIdentity);return {profile:shared.profile,guide:local.guide}}
async function applySharedIdentity(identity){sharedIdentity=normalizeIdentity({profile:identity?.profile||identity,guide:IDENTITY_DEFAULT.guide});await renderIdentityPresence();return sharedIdentity}
...
function renderExplore(s){const m=marks.find(x=>x.id===s.landmark)||marks[0],i=Math.min(2,s.step||0),language=s.language||"ko",p=promptFor(s.landmark,i,language);$("#exploreTitle").textContent="탐험 진행 · "+m.title;$("#question").textContent=p[0];$("#hint").textContent=p[1];$("#answer").value=s.answers[i]||"";setModeButtons(language);resolvedIdentity().then(identity=>{const name=guideDisplayName(identity),base=guideReaction(identity,s.landmark),stepLine=language==="en"?["Start small. One idea is enough.","Add one more piece.","Finish it in your own words."][i]:["작은 조각 하나부터 잡아보자.","좋아, 하나만 더 붙여보자.","이제 네 말로 마무리해보자."][i];$(".guideLine b").textContent=`길잡이 ${name}`;$("#guideLine").textContent=`${base} ${stepLine}`});$("#nextBtn").textContent=i===2?(language==="en"?"Finish exploration":"탐험 완료"):(language==="en"?"Next step":"다음 단계");$("#steps").innerHTML=STEPS.map((x,n)=>`<span class="${n===i?"on":n<i?"done":""}">${n+1}. ${x}</span>`).join("")}
...
$("#cloudBtn").onclick=()=>{const panel=$("#cloudPanel"),chips=$("#cloudChips"),frags=cloudFragments($("#answer").value);chips.innerHTML=frags.map(x=>`<button type="button">${html(x)}</button>`).join("");panel.hidden=false;chips.onclick=e=>{const b=e.target.closest("button");if(!b)return;resolvedIdentity().then(identity=>toast(`${guideDisplayName(identity)}: 힌트는 여기까지. 이제 네 문장으로 이어가봐.`))}};
...
async function renderIdentityPresence(){if(!db)return;const identity=await resolvedIdentity(),visual=$("#homeCharacterVisual"),rule=guideRule(identity),name=guideDisplayName(identity);$("#homeCharacterName").textContent=identity.profile.name||"나의 탐험가";$("#homeGuideName").textContent=name;$("#homeGuideLine").textContent=rule.home||"";const guideVisual=$("#homeGuide .guidePlaceholder");if(guideVisual)guideVisual.textContent=rule.label;if(identity.profile.photo){visual.innerHTML=`<img src="${identity.profile.photo}" alt="">`}else visual.textContent=(identity.profile.name||"탐험가").slice(0,2);$("#growthCompanion").textContent=`${identity.profile.name||"탐험가"} · ${name}와 함께 성장 중`;$("#guidePersonalityPreview").textContent=`${rule.label} · ${rule.personality||""}`}
async function renderGrowth(){if(!db)return;const cfg=await fetch("data/growth.json").then(r=>r.json()),exp=await get("exp")||0,p=levelProgress(exp);let stage=cfg[0];cfg.forEach(x=>{if(p.level>=x.min)stage=x});$("#growthLv").textContent="Lv."+p.level;$("#growthName").textContent=stage.name;$("#treeImage").src="assets/growth/"+stage.image;$("#expBar").style.width=(p.within*100)+"%";$("#expText").textContent=p.level>=25?`EXP ${exp} · 최고 성장 단계`:`EXP ${exp} · 다음 성장까지 ${p.remaining} EXP`;await renderIdentityPresence()}
...
async function renderLastResult(){const r=await get("lastResult");if(!r){$("#resultTitle").textContent="아직 완료한 탐험이 없어요.";$("#resultDraft").textContent="지도에서 탐험을 시작해봐요.";$("#resultExp").textContent="+0 EXP";$("#resultGem").textContent="보석 조각 +0";$("#resultStrengths").innerHTML="";$("#resultGuideLine").textContent="";$("#bonusStart").disabled=true;return}const m=marks.find(x=>x.id===r.landmark),bonusEvents=await get("bonusEvents")||{},identity=await resolvedIdentity();$("#resultTitle").textContent=m?.title||"오늘의 탐험";$("#resultDraft").textContent=r.answers.join(" ");$("#resultExp").textContent=`+${r.expAward||0} EXP`;$("#resultGem").textContent="보석 조각 +1";$("#resultStrengths").innerHTML=(r.strengths||[]).length?(r.strengths||[]).map(s=>`<span>${html(s)}</span>`).join(""):'<span>내 문장으로 끝까지 완성하기</span>';$("#resultGuideLine").textContent=`${guideDisplayName(identity)} · 네가 쓴 건 네 글이야. 나는 옆에서 길만 같이 봤어.`;const done=!!bonusEvents[`bonus_${r.completionEventId}`];$("#bonusStart").disabled=done;$("#bonusStart").textContent=done?"추가 연습 완료됨":"추가 연습";$("#bonusPanel").hidden=true}
...
async function renderSpecialInvite(){const invite=$("#specialInvite");if(!invite)return;invite.hidden=!isWeekend();if(!invite.hidden){const identity=await resolvedIdentity();invite.textContent=`${guideDisplayName(identity)}의 특별 탐험 초대장`}}
async function openSpecial(){const p=specialPromptFor(),identity=await resolvedIdentity();$("#specialDate").textContent=new Date().toLocaleDateString("ko-KR");$("#specialPrompt").textContent=p.q;$("#specialHint").textContent=`${guideDisplayName(identity)} · ${p.h}`;const draft=await get("specialDraft")||"";$("#specialAnswer").value=draft;show("special")}
