...
### 거점의 살아있는 상태
- 거점 탐험대원은 항상 같은 자리에 고정된 NPC가 아니다.
- 부재 사유는 생활형(배 아픔·휴가·늦잠 등), 업무형(다른 거점 지원·탐험 파견), 미스터리형(행방 불명·스페셜 관련)으로 변주할 수 있다.
- 부재 시 다른 탐험대원이 대타로 등장할 수 있다.
- 탐험 파견 후 복귀 시 새 지역 소문, 스페셜 흔적, 다음 챕터 힌트, 새 거점 이야기 등을 가져올 수 있다.
- 시스템상의 랜덤/주기 로직은 아이에게 확률로 노출하지 않고 **세계 안의 사건**으로 보여준다.
- 원래 거점 탐험대원이 돌아왔을 때 이전 부재/대타 사건을 기억하는 후속 대사를 가질 수 있다.

### Special Crew Encounter와 Special Exploration 분리
- **Special Crew Encounter**는 스페셜 탐험대원을 만나는 캐릭터/관계 시스템이다.
- **Special Exploration**은 선택형 글쓰기 이벤트다.
- 두 시스템은 보상·기록·상태 키를 공유하지 않는다.
- Special Crew Encounter는 흔적 → 만남 가능 상태 → 랜덤/거점 조우 → 첫 만남 → 도감 등록 → 메인 선택 가능 → 재방문의 흐름을 갖는다.
- 첫 만남은 단순 해금 팝업보다 현재 메인 탐험대원·거점 탐험대원·스페셜 탐험대원이 섞인 짧은 Micro Episode로 표현할 수 있다.
...
- 탐험대원 이름, 친밀도, 함께한 추억은 대원 교체 후에도 보존한다.
- 친밀도는 대사 변주·추억 회상·재회 표현에만 영향을 줄 수 있다.
- 친밀도가 학습 난이도, EXP, 보석, 기능 접근, 평가 결과에 영향을 주면 FAIL이다.
- 메인 탐험대원을 바꿔도 기록·성장·보석·소원·세션 이력은 초기화하지 않는다.

## 8C. 충돌/OPEN 관리

- **SUPERSEDED:** 과거의 `Special Guest 20명 별도 풀`은 최신 사용자 직접 정정인 `기본 탐험대 로스터 20명 + 스페셜 최대 12명 이하`와 충돌하므로 현행 기준으로 사용하지 않는다.
- **OPEN:** 시작 후보 5·6번째 개별 정체성.
- **OPEN:** 세계/거점 탐험대원의 개별 정체성.
- **OPEN:** 스페셜 탐험대원의 개별 정체성.
- **OPEN:** 스페셜의 정확한 해금 일수/챕터/거점 수치.
- OPEN 항목은 임의 보완하지 않고 슬롯/상태만 구현한다.
